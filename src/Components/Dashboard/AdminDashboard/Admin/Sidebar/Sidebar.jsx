...
const Sidebar = () => {
  ...
  return (
    <div className="flex sticky bg-white -mt-20">
      <div className={` ${open ? "w-72 h-full min-h-screen" : "w-20 h-full min-h-screen"} bg-[#EBEFEE] sticky p-5 duration-300 z-0`} >
        <img src="https://i.ibb.co/CB1yQwj/kuba-arrow-button-set-2.png" className={`absolute cursor-pointer -right-3 top-9 w-9 border-dark-purple border-2 rounded-full z-10 ${!open && "rotate-180"}`} onClick={handleOpenBar} alt="" />
        <div className="flex flex-col gap-x-4 items-center">
          ...
          <h3 className={`cursor-pointer text-center w-full text-2xl font-bold text-rose-600 mx-auto opacity-0 duration-500 ${open && " w-2/3 rotate-[360deg] opacity-100"}`} > Travel Guide</h3> <br />
        ...
        <div className={`flex rounded-md p-2 my-10 cursor-pointer hover:bg-gray-500 hover:text-white hover:bg-light-white text-gray-500 text-sm items-center gap-x-4  `}>
          <a href='/' className={`${!open && "hidden"} text-md flex items-center font-semibold origin-left duration-200`}> <FaHome className='text-2xl pr-2' />  Back to Homepage </a>
          <a href='/' className={`${open && "hidden"} text-md flex items-center font-semibold origin-left duration-200`}> <FaHome className='text-2xl pr-2' /></a>
        </div>
      </div>
